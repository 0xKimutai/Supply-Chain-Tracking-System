...
contract SupplyChainTest is Test {
    ...
    event StatusChanged(
        bytes32 indexed productId,
        SupplyChain.ProductStatus indexed oldStatus,
        SupplyChain.ProductStatus indexed newStatus,
        uint256 timestamp
    );
    ...
    function setUp() public {
        ...
        bytes memory initData = abi.encodeWithSelector(SupplyChain.initialize.selector, admin);
    ...
    function test_UpdateLocation_ManufacturerToInTransit() public {
        ...
        emit LocationUpdated(
            TEST_PRODUCT_ID, manufacturer, TEST_LOCATION, SupplyChain.ProductStatus.InTransit, block.timestamp
        );

        vm.expectEmit(true, true, true, true);
        emit StatusChanged(
            TEST_PRODUCT_ID, SupplyChain.ProductStatus.Created, SupplyChain.ProductStatus.InTransit, block.timestamp
        );
    ...
    function test_TransferOwnership_Unauthorized() public {
        ...
        vm.startPrank(unauthorized);
        vm.expectRevert(abi.encodeWithSelector(SupplyChain.UnauthorizedTransfer.selector, unauthorized, manufacturer));
    ...
    function test_Gas_ProductCreation() public {
        ...
        assertLt(gasUsed, 200_000); // Should use less than 200k gas
